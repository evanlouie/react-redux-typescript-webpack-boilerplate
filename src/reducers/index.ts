...
const initialState = {
  ...
  including: ["React", "React Redux", "React Router", "React Router Redux", "Redux", "TypeScript2", "Webpack2"],
  jokes: [],
  randomJunk: {
    foo: "bar"
  }
...
function greetings(state = initialState, action: any) {
  switch (action.type) {
    case ADD_MESSAGE:
      return {
        ...state,
        including: [...state.including, action.text]
      };
    case ADD_JOKE:
      return {
        ...state,
        jokes: [...state.jokes, action.text]
      };
    default:
      return state;
  }
